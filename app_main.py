...
def calculate_shap_values(
    ...
    if not SHAP_AVAILABLE:
        ...
        model.eval()
        with torch.no_grad():
            for si in range(n_perturb):
                seq = X_data_3d[si]  # (seq_length, n_features)
                seq_t = torch.tensor(seq, dtype=torch.float32).unsqueeze(0).to(device)
                base_pred = model(seq_t).cpu().numpy()[0]
                for j in range(n_features):
                    perturbed = seq.copy()
                    perturbed[:, j] = 0
                    p_t = torch.tensor(perturbed, dtype=torch.float32).unsqueeze(0).to(device)
                    p_pred = model(p_t).cpu().numpy()[0]
                    diff = base_pred - p_pred
                    feature_importance[j] += abs(diff)
                    shap_vals_agg[si, j] = diff
        feature_importance /= n_perturb
        for si in range(n_perturb, max_samples):
            shap_vals_agg[si] = shap_vals_agg[np.random.randint(0, n_perturb)]
        mx = feature_importance.max()
        fi_norm = (feature_importance / mx) if mx > 0 else feature_importance
        return fi_norm, shap_vals_agg, X_data_3d, feature_names, 'computed'
